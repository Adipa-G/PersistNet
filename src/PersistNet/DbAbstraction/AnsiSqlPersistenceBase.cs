...
internal abstract class AnsiSqlPersistenceBase : IDbPersistence
{
    ...
    /// <summary>
    /// Loads a single navigation property by delegating to the batch path with a
    /// one-element list, then extracting the single result.
    /// </summary>
    public async Task<object?> LoadNavigationAsync(
        object entity, Table entityTable, Relationship relationship,
        CancellationToken ct = default)
    {
        var result = await LoadNavigationBatchAsync(new[] { entity }, entityTable, relationship, ct);
        var key    = result.EntityKeySelector(entity);
        return key is not null && result.Entries.TryGetValue(key, out var value) ? value : null;
    }
